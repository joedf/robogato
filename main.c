...
int main(int argc, char *argv[]) {
	...
	const char *help   = "Current Commands : slap, eat, nom, ahk, g, uptime, ver[sion], say, cmd, dance, party, quit, restart, nick, chan, help";
		...
		do {
			...
			if (e > 0)
			{
				...
				if (sscanf(recvbuf,"PING :%s\r\n",suffix) > 0) {
					...
					if ((z=strpos(":!cmd",buffer))>0)
					{
						if ((5+z)>=strlen(buffer))
							continue;
						strtrim_l(buffer,5+z,buffer);
						printf("[REQUEST-CMD]-> <%s> %s\n",suffix,buffer);
						if ((!strcmp(suffix,master) || instrf(recvbuf,"%s PRIVMSG %s :!cmd",master,nick)) && (irc_sender(recvbuf,master)))
						{
							sck_sendf(&sIRC,"%s\r\n",buffer);
							#if (SAVELOG)
